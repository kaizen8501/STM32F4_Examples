...
uint32_t TwRingBufferWrite(TW_RING_BUFFER_t *ringBuffer, const uint8_t *data, uint32_t dataLength)
{
	...
	uint32_t amountToCopy = MIN(dataLength, (tailToEnd + ringBuffer->HEAD - 1) % ringBuffer->SIZE);

	memcpy(&ringBuffer->BUFF[ringBuffer->TAIL], data, MIN(amountToCopy, tailToEnd));

	if (tailToEnd < amountToCopy)
	{
		memcpy(&ringBuffer->BUFF[0], data + tailToEnd, amountToCopy - tailToEnd);
	}
